 const API_KEY = "AIzaSyCoE4psjYwGvQ4Vgl6Yk93J3iBGvvQdILk"; // ⚠️ Replace with valid Gemini API key
    ...
    function formatAnswerWithCode(text) {
      ...
      while ((match = codeBlockRegex.exec(text)) !== null) {
        ...
        // Use unescaped data-code attribute but escape in HTML output for safety
    ...
    function createMessage(text, isUser = false, skeleton = false) {
      ...
      // Different class for assistant vs user for styling
      div.className = "message " + (isUser ? "user" : "assistant");
      if (skeleton) div.classList.add("bot-skeleton");

      // Assistant has avatar + bubble; user keeps simple text
      if (!isUser) {
        ...
        // append inside bubble for better alignment
        bubble.appendChild(btn);

        // After insertion, wire up code copy buttons and Prism highlighting
        requestAnimationFrame(() => {
          ...
          // Prism highlight any pre tags inside this message
...
Focus on being helpful and practical. Use markdown formatting for better readability.`;
    ...
    function enhanceUserPrompt(userText) {
      ...
      // If user is asking for code, enhance the prompt
...
Please provide a complete, working solution with clear explanations and best practices.`;
          ...
          if (!res.ok) {
            ...
            if (data.error?.message?.includes("overloaded") || data.error?.message?.includes("quota")) {
              if (attempt < maxRetries) {
                ...
                delay *= 1.2; // Shorter backoff for faster retries
                continue;
              }
            }
    ...
    async function sendMessage() {
      ...
      try {
        ...
        // Enhance the user prompt for better responses
        const enhancedText = enhanceUserPrompt(text);
        
        // Add the enhanced user message to the conversation context
        conversationContext.push({ role: "user", parts: [{ text: enhancedText }] });
        
        // Create the request with system prompt if it's the first message
        const requestBody = {
          contents: conversationContext,
        };
        
        // Add system instruction for the first message to set context
        ...
        // Improve code formatting in the response
        ...
        // Limit conversation context to last 10 messages to avoid token limits
...
Please try your request again in a few minutes when the API is less busy.`;
        ...
        // If it's an overload error, also show helpful fallback information
    ...
    (function setupVoice() {
      ...
      // Auto-send when final result captured
      recognition.addEventListener("result", (e) => {
        const last = e.results[e.results.length - 1];
        if (last && last.isFinal) {
          setTimeout(() => { if (inputEl.value.trim()) sendMessage(); }, 250);
        }
      });
    })();
    ...
    document.getElementById("showHistory").addEventListener("click", showHistoryPanel);
